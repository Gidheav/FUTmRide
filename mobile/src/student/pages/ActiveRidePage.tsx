...
export default function ActiveRidePage({ rideId, onBack, onRideEnded }: ActiveRidePageProps) {
    ...
    if (next.length === 4) {
      ...
      try {
        ...
        try {
          ...
          useWalletStore.getState().syncBalance()
          if (onRideEnded) onRideEnded(); else onBack()
        } catch (err: any) {
  ...
  return (
    <View style={[styles.page, { paddingTop: insets.top }]}>
      ...
      >
        <Text style={pinStyles.bsTitle}>Confirm Cancellation</Text>
        <Text style={pinStyles.bsSubtitle}>Enter your Transaction PIN to cancel this ride.</Text>
