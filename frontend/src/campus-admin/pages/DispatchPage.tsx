...
export default function DispatchPage() {
  ...
  useEffect(() => {
    setWsConnected(rideWsConnected || fleetWsConnected)
  }, [rideWsConnected, fleetWsConnected, setWsConnected])
  ...
  useEffect(() => {
    ...
    const loadIncidents = async () => {
      try {
        const res = await api.get('tracking/fleet/incidents/')
        if (!isActive) return
    ...
    void loadIncidents()
    timer = setInterval(loadIncidents, 30000)
      ...
      <div style={s.centerPanel}>
        ...
        <div style={s.mapArea}>
          {!isLoaded ? (
            ...
            >
              {fleetList.slice(0, 500).map((driver) => {
          ...
          {showHeat && <div style={s.heatOverlay} />}
        </div>
      </div>
      ...
      {isRightPanelOpen ? (
        <div style={s.rightPanel}>
          <div style={s.rpHeader}>
            ...
            <span style={{ ...s.wsBadge, color: rideWsConnected || fleetWsConnected ? T.heatTeal : T.warn }}>
              <Radio size={12} />
              {rideWsConnected || fleetWsConnected ? 'Live' : 'Offline'}
            </span>
