...
const labStyles: Record<string, any> = {
  ...
  // Area Navigation
  areaNav: { display: 'flex', gap: 2, background: T.bgInput, padding: 4, borderRadius: 8 },
  areaNavTab: (active: boolean): CSSProperties => ({
    display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', fontSize: 12, fontWeight: 600, fontFamily: T.fontFamily, cursor: 'pointer', color: active ? T.textPrimary : T.textMuted, background: active ? T.bgCard : 'transparent', border: 'none', borderRadius: 6, transition: 'all 0.15s',
  }),

  // Main Layout (matching RouteOpsPanel)
  mainLayout: { display: 'flex', flex: 1, overflow: 'hidden', gap: 2 },
  leftCol: { width: 280, display: 'flex', flexDirection: 'column', overflow: 'auto', gap: 2, flexShrink: 0 },
  ...
  // Tools Grid
  toolsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 8 },
  toolCard: { background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 8, padding: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, transition: 'all 0.15s', },
  toolCardActive: { borderColor: '#a855f7', background: 'rgba(168,85,247,0.08)', borderWidth: 2 },
  toolCardContent: { flex: 1, minWidth: 0 },
  toolCardTitle: { fontSize: 12, fontWeight: 600, color: T.textPrimary, marginBottom: 2 },
  toolCardCount: { fontSize: 18, fontWeight: 800, color: '#a855f7' },

  // Premium Panel
  premiumPanel: { background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 0, padding: 24, display: 'flex', flexDirection: 'column', gap: 20 },

  // Panel Header
  panelHeader: { display: 'flex', alignItems: 'flex-start', gap: 16, paddingBottom: 16, borderBottom: `1px solid ${T.border}` },
  panelIconWrapper: { width: 48, height: 48, borderRadius: 12, background: 'rgba(168,85,247,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a855f7', flexShrink: 0 },
  panelHeaderContent: { flex: 1, minWidth: 0 },
  panelTitle: { margin: 0, fontSize: 16, fontWeight: 700, color: T.textPrimary, letterSpacing: -0.3, marginBottom: 4 },
  panelDescription: { margin: 0, fontSize: 12, color: T.textMuted, lineHeight: 1.5 },

  // Stats Grid
  statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 },
  statCard: { background: T.bgPanel, border: `1px solid ${T.border}`, borderRadius: 8, padding: 12, display: 'flex', alignItems: 'center', gap: 10 },
  statIcon: { width: 36, height: 36, borderRadius: 8, background: 'rgba(168,85,247,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a855f7', flexShrink: 0 },
  statContent: { flex: 1, minWidth: 0 },
  statValue: { fontSize: 18, fontWeight: 800, color: T.textPrimary, marginBottom: 2 },
  statLabel: { fontSize: 10, color: T.textMuted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 },

  // Action Section
  actionSection: { display: 'flex', flexDirection: 'column', gap: 16 },
  fieldsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 },
  inputGroup: { display: 'flex', flexDirection: 'column', gap: 6 },
  inputLabel: { fontSize: 10, color: T.textMuted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 },
  premiumInput: { background: T.bgInput, border: `1px solid ${T.border}`, color: T.textPrimary, borderRadius: 6, padding: '10px 14px', fontSize: 13, outline: 'none', width: '100%', fontFamily: T.fontFamily, boxSizing: 'border-box' },
  premiumSelect: { background: T.bgInput, border: `1px solid ${T.border}`, color: T.textPrimary, borderRadius: 6, padding: '10px 14px', fontSize: 13, outline: 'none', width: '100%', fontFamily: T.fontFamily, boxSizing: 'border-box', cursor: 'pointer' },

  // Button Group
  buttonGroup: { display: 'flex', gap: 8, flexWrap: 'wrap' },
  primaryButton: { display: 'flex', alignItems: 'center', gap: 6, background: '#a855f7', color: '#ffffff', border: 'none', padding: '10px 20px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: T.fontFamily, transition: 'all 0.15s' },
  dangerButton: { display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', border: `1px solid rgba(239, 68, 68, 0.3)`, padding: '10px 20px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: T.fontFamily },
  flushButton: { display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(234, 179, 8, 0.1)', color: '#eab308', border: `1px solid rgba(234, 179, 8, 0.3)`, padding: '10px 20px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: T.fontFamily },

  // Verify Section
  verifySection: { display: 'flex', flexDirection: 'column', gap: 20 },
  verifyHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: T.bgPanel, borderRadius: 8, border: `1px solid ${T.border}` },
  verifyTitle: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 600, color: T.textPrimary },
  verifyCount: { fontSize: 11, fontWeight: 600, color: '#a855f7', background: 'rgba(168,85,247,0.1)', padding: '4px 10px', borderRadius: 4 },
  emptyState: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, padding: 32, color: T.textMuted, fontSize: 12, background: T.bgPanel, borderRadius: 8, border: `2px dashed ${T.border}` },
  tableContainer: { background: T.bgPanel, borderRadius: 8, border: `1px solid ${T.border}`, overflow: 'hidden' },
  premiumTable: { width: '100%', borderCollapse: 'collapse' },
  th: { textAlign: 'left', padding: '12px 16px', fontSize: 10, fontWeight: 700, color: T.textMuted, textTransform: 'uppercase', letterSpacing: 0.5, borderBottom: `1px solid ${T.border}`, background: T.bgInput },
  td: { padding: '12px 16px', fontSize: 12, color: T.textPrimary, borderBottom: `1px solid ${T.border}` },
  statusBadge: { padding: '4px 10px', borderRadius: 4, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, border: '1px solid' },
  ...
  logList: { flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, minHeight: 0 },
  logEntry: { display: 'flex', gap: 8, alignItems: 'flex-start', padding: '8px 0' },
  logDot: { width: 6, height: 6, borderRadius: 3, flexShrink: 0, marginTop: 4 },
  logMsg: { fontSize: 12, color: T.textPrimary, lineHeight: 1.3, marginBottom: 2 },
  logTime: { fontSize: 10, color: T.textMuted },
  logJson: { margin: 0, fontSize: 10, color: T.textSecondary, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace', whiteSpace: 'pre-wrap', wordBreak: 'break-word' },
}